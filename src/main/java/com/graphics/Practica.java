...
public class Practica {
    ...
    private static final float VELOCIDAD = 1.9f; // Velocidad de movimiento del triángulo (en unidades por segundo)
    private float offsetX = 0.0f; // Desplazamiento horizontal actual del triángulo
    private float offsetY = 0.0f; // Desplazamiento vertical actual del triángulo
    private static final float LIMITE = 0.5f; // Límite de movimiento (el triángulo no se moverá más allá de este valor)
    /**
    ...
    private void init() {
        ...
        GLFW.glfwSetKeyCallback(window, (win, key, scancode, action, mods) -> {
            if (key == GLFW.GLFW_KEY_ESCAPE && action == GLFW.GLFW_PRESS) {
                GLFW.glfwSetWindowShouldClose(win, true); // Cerrar la ventana al pulsar ESC
            }
        });
